...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'metadata' | 'blogs' | 'redirects' | 'trackers' | 'leads'>('overview');
  ...
  const [perfScores, setPerfScores] = useState<Record<string, {
    mobile: number | null;
    desktop: number | null;
    loading: boolean;
    simulated: boolean;
  }>>({
    gotolatest: { mobile: null, desktop: null, loading: false, simulated: false },
  ...
  const fetchMetadata = async (path: string) => {
    ...
    setMetaMessage(null);
    try {
      const res = await fetch(`/api/seo-config?siteId=${selectedSite}&path=${path}`);
      if (res.ok) {
        const data = await res.json();
        setMetaForm({
          path: path,
  ...
  const fetchBlogs = async () => {
    ...
    resetBlogForm();
    try {
      const res = await fetch(`/api/blogs?siteId=${selectedSite}&publishedOnly=false`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleMetaSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setMetaMessage({ type: 'success', text: 'On-Page SEO Configuration updated successfully!' });
        fetchStats(); // Update dashboard counts
      } else {
        const err = await res.json();
        setMetaMessage({ type: 'error', text: err.error || 'Failed to update SEO config.' });
      }
    } catch (err) {
  ...
  const handleBlogSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint = selectedBlog ? `/api/blogs/${selectedBlog.slug}` : '/api/blogs';
      ...
      if (res.ok) {
        setBlogMessage({
          type: 'success',
          text: selectedBlog ? 'Blog article updated successfully!' : 'Blog article published successfully!',
        });
        fetchBlogs(); // Reload blog list
        fetchStats(); // Update dashboard counts
      } else {
        const err = await res.json();
        setBlogMessage({ type: 'error', text: err.error || 'Failed to save blog post.' });
      }
    } catch (err) {
  ...
  const handleBlogDelete = async (slug: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setBlogMessage({ type: 'success', text: 'Blog post deleted successfully.' });
        fetchBlogs();
        fetchStats();
      }
    } catch (err) {
  ...
  const handleRedirectSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        setRedirectMessage({ type: 'error', text: err.error || 'Failed to save redirect rule.' });
      }
    } catch (err) {
  ...
  const handleRedirectDelete = async (id: string) => {
    try {
      ...
      setRedirectMessage({ type: 'error', text: 'Failed to delete redirect rule.' });
    }
  };
  ...
  const handleTrackerSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setTrackerMessage({ type: 'success', text: 'Pixel and Script headers updated!' });
        fetchStats(); // Update pixel indicator light on dashboard
      } else {
        setTrackerMessage({ type: 'error', text: 'Failed to update script headers.' });
      }
    } catch (err) {
    ...
    <div className="flex-1 flex flex-col md:flex-row h-screen overflow-hidden">
      ...
      <aside className="w-full md:w-64 bg-slate-900 border-b md:border-b-0 md:border-r border-slate-800 flex flex-col shrink-0">
        ...
        <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-800">
          ...
          <div>
            <h1 className="font-extrabold text-lg tracking-tight bg-gradient-to-r from-white via-indigo-200 to-indigo-400 bg-clip-text text-transparent">Antigravity</h1>
            <p className="text-[10px] text-indigo-400 font-bold tracking-widest uppercase">SEO Command Center</p>
          </div>
        </div>

        {/* WEBSITE GLOBAL SELECTOR */}
        <div className="p-4 border-b border-slate-850">
          <label className="block text-[10px] uppercase font-bold tracking-wider text-slate-500 mb-1.5 px-1">Selected Site</label>
        ...
        <div className="flex-1 p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {loadingStats ? (
                  ...
                  siteStatsList.map((site) => {
                    ...
                    const hasScores = scores && scores.mobile !== null && scores.desktop !== null && !scores.loading;
                    return (
                      <div
                        key={site.id}
                      ...
                      >
                        <div>
                          ...
                          <div className="flex items-start justify-between">
                            <div>
                              <h3 className="font-bold text-lg text-white tracking-tight">{site.name}</h3>
                              <a
                                href={site.baseUrl}
                                target="_blank"
                                rel="noreferrer"
                            ...
                            <div className="flex gap-2">
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${site.pixels.ga4 ? 'bg-emerald-500/15 text-emerald-400' : 'bg-slate-800 text-slate-500'}`}>
                                GA4
                              </span>
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${site.pixels.meta ? 'bg-purple-500/15 text-purple-400' : 'bg-slate-800 text-slate-500'}`}>
                                PIXEL
                              </span>
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-4 mt-6">
                            <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-850">
                              <div className="text-xs text-slate-400 font-medium">SEO Pages</div>
                              <div className="text-xl font-extrabold text-white mt-0.5">{site.counts.seoConfigs}</div>
                            </div>
                            <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-850">
                              <div className="text-xs text-slate-400 font-medium">CMS Blogs</div>
                              <div className="text-xl font-extrabold text-white mt-0.5">{site.counts.blogs}</div>
                            </div>
                            <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-850">
                              <div className="text-xs text-slate-400 font-medium">Redirects</div>
                              <div className="text-xl font-extrabold text-white mt-0.5">{site.counts.redirects}</div>
                            </div>
                            <div className="bg-slate-950/50 rounded-xl p-3 border border-slate-850">
                              <div className="text-xs text-slate-400 font-medium">CRM Leads</div>
                              <div className="text-xl font-extrabold text-white mt-0.5">{site.counts.leads}</div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="mt-4 border-t border-slate-800/80 pt-3 flex flex-col gap-2 text-xs">
                          <div className="flex justify-between items-center">
                            ...
                            <div className="flex gap-3 font-extrabold">
                              {scores?.loading ? (
                                <span className="text-indigo-400 animate-pulse">Auditing...</span>
                              ) : hasScores ? (
                                <>
                                  <span className={(scores.mobile ?? 0) >= 90 ? 'text-emerald-400' : (scores.mobile ?? 0) >= 50 ? 'text-amber-400' : 'text-red-400'}>
                                    Mobile: {scores.mobile}
                                  </span>
                                  <span className={(scores.desktop ?? 0) >= 90 ? 'text-emerald-400' : (scores.desktop ?? 0) >= 50 ? 'text-amber-400' : 'text-red-400'}>
                          ...
                          <button
                            ...
                            onClick={() => runPerformanceAudit(site.id, site.baseUrl)}
                          ...
                          >
                            {scores?.loading ? 'Analyzing PageSpeed...' : 'Run Live Lighthouse Audit'}
                          </button>
                ...
                <div className="glass-panel rounded-2xl p-6 border border-slate-800">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3.5 bg-slate-950/40 rounded-xl border border-slate-850">
                      <div>
                        <div className="text-sm font-semibold text-slate-200">Next.js Client Module</div>
                        <div className="text-xs text-slate-500 mt-0.5">gotolatest, sunnest</div>
                      </div>
                    ...
                    <div className="flex justify-between items-center p-3.5 bg-slate-950/40 rounded-xl border border-slate-850">
                      <div>
                        <div className="text-sm font-semibold text-slate-200">React Client Header Injection</div>
                        <div className="text-xs text-slate-500 mt-0.5">hehe (Raipur Podcast)</div>
                      </div>
                      <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
                        Pre-rendering Set
                      </span>
                    ...
                    <div className="flex justify-between items-center p-3.5 bg-slate-950/40 rounded-xl border border-slate-850">
                      <div>
                        <div className="text-sm font-semibold text-slate-200">Central SQLite Database</div>
                        <div className="text-xs text-slate-500 mt-0.5">Prisma ORM</div>
                      </div>
                ...
                <div className="glass-panel rounded-2xl p-6 border border-slate-800 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mb-4">
                      Leads and customer sign-ups captured from forms on your three sites route here.
                    </p>
                  </div>
                  
                  <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-850 text-center py-6">
                    <h4 className="text-sm text-slate-400 font-semibold uppercase tracking-wider">Total Captured Contacts</h4>
                    <p className="text-4xl font-extrabold text-white mt-1.5">
                      {siteStatsList.reduce((acc, curr) => acc + curr.counts.leads, 0)}
                    </p>
                    <button
                      onClick={() => setActiveTab('leads')}
            ...
            <div className="glass-panel rounded-2xl p-6 border border-slate-800">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-4 pb-6 mb-6 border-b border-slate-800">
                <div className="flex-1">
                  <label className="block text-xs font-bold text-slate-400 mb-1.5">Edit Route Pathname</label>
                  <div className="relative">
              ...
              {metaMessage && (
                <div className={`p-4 mb-6 rounded-xl border flex items-start gap-2.5 text-sm ${
                  metaMessage.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'
                }`}>
                  {metaMessage.type === 'success' ? <CheckCircle className="h-5 w-5 shrink-0 mt-0.5" /> : <AlertCircle className="h-5 w-5 shrink-0 mt-0.5" />}
                  {metaMessage.text}
                </div>
              )}
              ...
              {loadingMeta ? (
                ...
                <form onSubmit={handleMetaSubmit} className="space-y-6">
                  ...
                  {/* On-Page Meta */}
                  <div className="space-y-4">
                    <h3 className="text-base font-bold text-white border-l-3 border-indigo-500 pl-2">A. On-Page Search Tags</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1.5">Canonical URL Override</label>
                        <input
                          type="text"
                          value={metaForm.canonicalUrl || ''}
                          onChange={(e) => setMetaForm(prev => ({ ...prev, canonicalUrl: e.target.value }))}
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1.5">Robots Directive</label>
                        <select
                          value={metaForm.robots || 'index, follow'}
                          onChange={(e) => setMetaForm(prev => ({ ...prev, robots: e.target.value }))}
                        ...
                        >
                          ...
                          <option value="noindex, nofollow">noindex, nofollow (Ignore Indexing & Links)</option>
                          <option value="noindex, follow">noindex, follow (Ignore Page, Follow Links)</option>
                        </select>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-800/80">
                    <h3 className="text-base font-bold text-white border-l-3 border-purple-500 pl-2">B. Social Graph Tags (OpenGraph & Twitter)</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1.5">OG Custom Title</label>
                        <input
                          type="text"
                          value={metaForm.ogTitle || ''}
                          onChange={(e) => setMetaForm(prev => ({ ...prev, ogTitle: e.target.value }))}
                          placeholder="OG Social Title (Falls back to Meta Title if blank)"
                          className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500"
                        />
                      </div>
                      <div>
                        <label className="block text-xs font-semibold text-slate-400 mb-1.5">OG Custom Image URL</label>
                        <input
                          type="text"
                          value={metaForm.ogImage || ''}
                          onChange={(e) => setMetaForm(prev => ({ ...prev, ogImage: e.target.value }))}
                          placeholder="Image link for social sharing prevews"
                          className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500"
                        />
                      </div>
                      <div className="md:col-span-2">
                        <label className="block text-xs font-semibold text-slate-400 mb-1.5">OG Custom Description</label>
                        <textarea
                          rows={2}
                          value={metaForm.ogDescription || ''}
                          onChange={(e) => setMetaForm(prev => ({ ...prev, ogDescription: e.target.value }))}
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-800/80">
                    <div className="flex justify-between items-center">
                      <h3 className="text-base font-bold text-white border-l-3 border-emerald-500 pl-2">C. Schema Markup (JSON-LD)</h3>
                      <button
                        type="button"
                        onClick={() => {
                          try {
                            if (metaForm.schemaMarkup) {
                              ...
                              setMetaForm(prev => ({ ...prev, schemaMarkup: JSON.stringify(parsed, null, 2) }));
                    ...
                    <div>
                      <textarea
                        ...
                        onChange={(e) => setMetaForm(prev => ({ ...prev, schemaMarkup: e.target.value }))}
                  ...
                  <div className="pt-4 border-t border-slate-800/85">
                    ...
                    >
                      {loadingMeta ? 'Saving Configuration...' : 'Save Page SEO configuration'}
                    </button>
                  </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-4 glass-panel rounded-2xl p-4 border border-slate-800 flex flex-col h-[600px]">
                <div className="flex justify-between items-center mb-4 pb-3 border-b border-slate-800">
                  <h3 className="font-bold text-white text-sm">Written Articles</h3>
                  <button
                    onClick={() => {
                      setSelectedBlog(null);
                      resetBlogForm();
                ...
                {loadingBlogs ? (
                  ...
                  <div className="flex-1 overflow-y-auto space-y-2 pr-1 scrollbar-hide">
                    {blogList.length === 0 ? (
                      <div className="text-center text-xs text-slate-500 py-12">No blogs created for this site yet.</div>
                    ) : (
                      blogList.map((blog) => (
                        <div
                          key={blog.id}
                        ...
                        >
                          ...
                          <div className="flex justify-between items-center mt-2 text-[10px] text-slate-400">
                            <span>Slug: <code className="font-mono text-indigo-400">/{blog.slug}</code></span>
                            <span className={`px-1.5 py-0.5 rounded-full font-bold uppercase ${blog.published ? 'bg-emerald-500/15 text-emerald-400' : 'bg-amber-500/15 text-amber-400'}`}>
                ...
                <div className="glass-panel rounded-2xl p-6 border border-slate-800">
                  ...
                  {blogMessage && (
                    <div className={`p-3.5 mb-6 rounded-xl border flex items-start gap-2.5 text-sm ${
                      blogMessage.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'
                    }`}>
                      {blogMessage.type === 'success' ? <CheckCircle className="h-5 w-5 mt-0.5" /> : <AlertCircle className="h-5 w-5 mt-0.5" />}
                      {blogMessage.text}
                    </div>
                  )}
                  ...
                  <form onSubmit={handleBlogSubmit} className="space-y-4">
                    ...
                    <div className="space-y-3 pt-4 border-t border-slate-800/80">
                      <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Custom Blog SEO Overrides</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-semibold text-slate-400 mb-1">Custom Title Tag</label>
                          <input
                            type="text"
                        ...
                        <div>
                          <label className="block text-[10px] font-semibold text-slate-400 mb-1">Custom OG Image URL</label>
                          <input
                            type="text"
                            value={blogForm.seoOgImage || ''}
                            onChange={(e) => setBlogForm(prev => ({ ...prev, seoOgImage: e.target.value }))}
                            placeholder="Defaults to Featured Image if empty"
                            className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-indigo-500"
                          />
                        </div>
                        <div className="md:col-span-2">
                          <label className="block text-[10px] font-semibold text-slate-400 mb-1">Custom Description Tag</label>
                          <input
                            type="text"
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between pt-4 border-t border-slate-800/80 gap-4">
                      <label className="flex items-center gap-2 cursor-pointer select-none">
                        ...
                        />
                        <span className="text-sm font-semibold text-slate-200">Publish immediately to website</span>
                      </label>
          ...
          {activeTab === 'redirects' && (
            <div className="space-y-6">
              ...
              {/* Creator Form */}
              <div className="glass-panel rounded-2xl p-6 border border-slate-800">
                <h3 className="font-bold text-white text-base mb-4 border-l-3 border-indigo-500 pl-2">Create New URL Redirect</h3>
                
                {redirectMessage && (
                  <div className={`p-3 rounded-xl border flex items-start gap-2 text-sm mb-4 ${
                    redirectMessage.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'
                  }`}>
                    {redirectMessage.type === 'success' ? <CheckCircle className="h-5 w-5 mt-0.5" /> : <AlertCircle className="h-5 w-5 mt-0.5" />}
                    {redirectMessage.text}
                  </div>
                )}

                <form onSubmit={handleRedirectSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 mb-1.5">Source URL Path</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. /old-page"
                      value={redirectForm.sourcePath}
                      onChange={(e) => setRedirectForm(prev => ({ ...prev, sourcePath: e.target.value }))}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 mb-1.5">Destination URL Path</label>
                    <input
                      type="text"
                      required
                      placeholder="e.g. /blog/new-page"
                      value={redirectForm.destinationPath}
                      onChange={(e) => setRedirectForm(prev => ({ ...prev, destinationPath: e.target.value }))}
                      className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500 font-mono"
                    />
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1">
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Redirect Code</label>
                      <select
                        value={redirectForm.statusCode}
                        onChange={(e) => setRedirectForm(prev => ({ ...prev, statusCode: parseInt(e.target.value, 10) }))}
                        className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-indigo-500 cursor-pointer"
                      >
              ...
              {/* Redirect List Table */}
              <div className="glass-panel rounded-2xl p-6 border border-slate-800">
                <h3 className="font-bold text-white text-base mb-4">Active Redirection Tables</h3>
                        ...
                        {redirectList.length === 0 ? (
                          <tr>
                            <td colSpan={4} className="p-8 text-center text-slate-500">
                              No redirects defined. Add a rule above to prevent crawler 404 penalties.
                            </td>
                          </tr>
                        ) : (
                          redirectList.map((rule) => (
                            <tr key={rule.id} className="hover:bg-slate-900/25">
                              <td className="p-4 font-mono text-indigo-400">{rule.sourcePath}</td>
                              <td className="p-4 font-mono text-slate-400">{rule.destinationPath}</td>
                              <td className="p-4">
                                <span className={`px-2.5 py-0.5 rounded-full font-bold ${
                                  rule.statusCode === 301 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-amber-500/10 text-amber-400'
                                }`}>
          ...
          {activeTab === 'trackers' && (
            <div className="glass-panel rounded-2xl p-6 border border-slate-800">
              ...
              <h3 className="font-bold text-white text-base mb-6 border-l-3 border-indigo-500 pl-2">Configure Pixel Tracking & Tag Management</h3>

              {trackerMessage && (
                <div className={`p-3.5 mb-6 rounded-xl border flex items-start gap-2.5 text-sm ${
                  trackerMessage.type === 'success' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border-red-500/30 text-red-400'
                }`}>
                  {trackerMessage.type === 'success' ? <CheckCircle className="h-5 w-5 mt-0.5" /> : <AlertCircle className="h-5 w-5 mt-0.5" />}
                  {trackerMessage.text}
                </div>
              )}
              ...
              {loadingTracker ? (
                ...
                <form onSubmit={handleTrackerSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Google Analytics (GA4) Measurement ID</label>
                      <input
                        type="text"
                        value={trackerForm.googleAnalyticsId}
                        onChange={(e) => setTrackerForm(prev => ({ ...prev, googleAnalyticsId: e.target.value }))}
                        placeholder="e.g. G-XXXXXXXXXX"
                        className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-indigo-500"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Meta (Facebook) Pixel ID</label>
                      <input
                        type="text"
                        value={trackerForm.metaPixelId}
                        onChange={(e) => setTrackerForm(prev => ({ ...prev, metaPixelId: e.target.value }))}
                        placeholder="e.g. 1234567890"
                        className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-indigo-500"
                      />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Google Search Console Verification Tag</label>
                      <input
                        type="text"
                        value={trackerForm.searchConsoleTag}
                        onChange={(e) => setTrackerForm(prev => ({ ...prev, searchConsoleTag: e.target.value }))}
                  ...
                  <div className="space-y-4 pt-4 border-t border-slate-800/80">
                    <h3 className="font-bold text-white text-sm">Custom Code Injection (Enterprise Integration)</h3>
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Header Scripts (`&lt;head&gt;` insertion)</label>
                      <textarea
                        rows={4}
                        value={trackerForm.headerScripts}
                        onChange={(e) => setTrackerForm(prev => ({ ...prev, headerScripts: e.target.value }))}
                        placeholder="<!-- Raw HTML like Hotjar scripts, custom styles, GTM tags -->"
                        className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-xs rounded-lg px-3 py-2.5 focus:outline-none focus:border-indigo-500 font-mono"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 mb-1.5">Footer Scripts (end of `&lt;body&gt;` insertion)</label>
                      <textarea
                        rows={4}
                        value={trackerForm.footerScripts}
                        onChange={(e) => setTrackerForm(prev => ({ ...prev, footerScripts: e.target.value }))}
                  ...
                  <div className="pt-4 border-t border-slate-800/85">
                    ...
                    >
                      {loadingTracker ? 'Updating configuration...' : 'Update tracking configuration'}
                    </button>
                  </div>
          ...
          {activeTab === 'leads' && (
            <div className="glass-panel rounded-2xl p-6 border border-slate-800">
              ...
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 pb-4 mb-6 border-b border-slate-800">
                <h3 className="font-bold text-white text-base">Captured Leads & Newsletter Signups</h3>
                <div className="flex items-center gap-2">
                  <label className="text-xs font-bold text-slate-400 shrink-0">Filter Site:</label>
                  <select
                    value={leadsFilter}
                    onChange={(e) => {
                      ...
                      {leadsList.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="p-8 text-center text-slate-500">
                            No leads captured yet. Connect your site's contact forms to POST to `http://localhost:3000/api/leads` to sync entries here.
                          </td>
                        </tr>
                      ) : (
                        leadsList.map((lead) => (
                          <tr key={lead.id} className="hover:bg-slate-900/25">
                            ...
                            <td className="p-4 font-bold text-white whitespace-nowrap">
                              {lead.name}
                              <span className="block text-[10px] text-indigo-400 font-semibold mt-0.5">{lead.formName}</span>
                            </td>
                            <td className="p-4 space-y-0.5 whitespace-nowrap">
                              <div className="text-slate-200">{lead.email}</div>
                              {lead.phone && <div className="text-[10px] text-slate-500 font-mono">{lead.phone}</div>}
                            </td>
                            <td className="p-4 max-w-xs truncate text-slate-300" title={lead.message}>
                              {lead.message || <span className="text-slate-600 italic">No message supplied</span>}
                            </td>
                            <td className="p-4 text-center whitespace-nowrap">
                              <span className={`px-2.5 py-0.5 rounded-full font-bold uppercase text-[9px] ${
                                lead.siteId === 'gotolatest' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20' :
                                lead.siteId === 'hehe' ? 'bg-purple-500/10 text-purple-400 border border-purple-500/20' :
